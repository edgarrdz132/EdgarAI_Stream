...
const TABS = [
  { id: '', name: 'Todas' },
  { id: '&with_original_language=es', name: 'En Español' },
  { id: '&with_original_language=en', name: 'En Inglés' },
  { id: ',10762', name: 'Familia' },
]
...
export default function CaricaturasPage() {
  ...
  useEffect(() => { load(tab, 1, true) }, [tab])

  async function load(t, p, reset) {
    setLoading(true)
    const d = await tmdbAPI.getCaricaturas(t, p)
    if (d?.results) {
  ...
  return (
    <div style={{ padding: '1.5rem 2.5rem' }}>
      ...
      <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '2rem', letterSpacing: 3, marginBottom: '1.25rem' }}>🎨 Caricaturas</div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            background: tab === t.id ? '#e50914' : 'transparent',
            color: tab === t.id ? '#fff' : '#888',
            border: tab === t.id ? '1px solid #e50914' : '1px solid rgba(255,255,255,0.1)',
            transition: 'all 0.2s',
          }}>{t.name}</button>
        ))}
      </div>
        ...
        <>
          ...
          {hasMore && (
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              <button onClick={() => load(tab, page + 1, false)} style={{
